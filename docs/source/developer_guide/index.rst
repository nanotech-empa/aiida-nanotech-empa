...
``aiida-nanotech-empa`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
The documentation of ``aiida-nanotech-empa``
is ready for `ReadTheDocs <https://readthedocs.org/>`_:

Simply add the ``aiida-nanotech-empa`` repository on your RTD profile, preferably using ``aiida-nanotech-empa`` as the project name - that's it!
