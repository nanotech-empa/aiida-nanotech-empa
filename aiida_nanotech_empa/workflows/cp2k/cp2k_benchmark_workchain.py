...
def get_timing_from_FolderData(folder_node=None):
    ...
    if "aiida.out" not in folder_node.list_object_names():
        raise FileNotFoundError(
            f"'aiida.out' not found in FolderData with uuid {folder_node.uuid}"
        )
...
class Cp2kBenchmarkWorkChain(engine.WorkChain):
    ...
    def setup(self):
        ...
        cutoff = getattr(
            self.inputs,
            "cutoff",
            cp2k_utils.get_cutoff(structure=self.inputs.structure),
        )
    ...
    def submit_calculations(self):
        ...
        for nnodes in self.inputs.list_nodes:
            # Loop for mpi tasks
            for ntasks in find_multiples_of_ngpus(
                self.inputs.ngpus.value, nnodes, self.inputs.max_tasks_per_node.value
            ):
        ...
        for nnodes in self.inputs.list_nodes:
            ...
            for ntasks in find_multiples_of_ngpus(
                ...
                for nthreads in self.inputs.list_threads_per_task:
                    ...
                    if nthreads <= self.ctx.max_tasks / ntasks:
                        current_calc = getattr(
                            self.ctx, f"run_{nnodes}_{ntasks}_{nthreads}"
                        )
