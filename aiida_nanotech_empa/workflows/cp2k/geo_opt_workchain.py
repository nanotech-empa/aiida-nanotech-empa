...
class Cp2kGeoOptWorkChain(WorkChain):
    ...
    def setup(self):
        ...
        # get cutoff
        self.ctx.cutoff = get_cutoff(structure=self.inputs.structure)
        ...
        if self.ctx.dft_params['periodic'] == 'NONE':
            ...
            self.ctx.input_dict['FORCE_EVAL']['DFT']['POISSON']['PSOLVER'] = 'MT'
        ...
        # cell optimization:
        if self.ctx.sys_params['cell_opt']:
            with open(
            ...
            for sym in ['KEEP_SYMMETRY', 'KEEP_ANGLES','KEEP_SPACE_GROUP']:
                if sym in self.ctx.sys_params:
                    self.ctx.input_dict['MOTION']['CELL_OPT'][sym] = sym
            for constraint in ['X','Y','Z','XY','XZ','YZ','XYZ','NONE']:
                if constraint in self.ctx.sys_params:
                    self.ctx.input_dict['MOTION']['CELL_OPT']['CONSTRAINT'][constraint] = constraint
