...
def check_symmetry(cell_lengths, angles):
    ...
    value_expand = (
        lambda variable: round(14 / variable) if round(14 / variable) != 0 else 1
    )
    super_cell = (
        f"{value_expand(a)} {value_expand(b)} {value_expand(c)}"  # 14 Angstrom cutoff
    )
...
class Cp2kDielectricWorkChain(WorkChain):
    ...
    def setup(self):
        ...
        with open(path_settings) as stream:
    ...
    def run_efield0(self):
        ...
        efield_parameters = {
            "FORCE_EVAL": {
                "DFT": {
                    ...
                    "PERIODIC_EFIELD": {"INTENSITY": 0.000, "POLARISATION": "1 0 0"},
                    "PRINT": {
                        "MOMENTS": {"PERIODIC": True},
                    },
                }
            }
        }
        dict_merge(self.ctx.efield_parameters, efield_parameters)
        efield0 = AttributeDict(
            self.exposed_inputs(Cp2kBaseWorkChain, namespace="cp2k_base")
        )
        ...
        efield0["cp2k"][
            "parent_calc_folder"
        ] = self.ctx.ground_state.outputs.remote_folder
    ...
    def results(self):
        ...
        eps_inf_xx = (
            4 * np.pi * (np.absolute(dipole_efieldx - dipole_efield0)) * 0.393456
        ) / (volume * np.power(1.8897259886, 3) * 0.0005) + 1
        eps_inf_yy = (
            4 * np.pi * np.absolute(dipole_efieldy - dipole_efield0) * 0.393456
        ) / (volume * np.power(1.8897259886, 3) * 0.0005) + 1
        eps_inf_zz = (
            4 * np.pi * np.absolute(dipole_efieldz - dipole_efield0) * 0.393456
        ) / (volume * np.power(1.8897259886, 3) * 0.0005) + 1

        eps_0_xx = (
            4 * np.pi * (np.absolute(dipole_efieldx_relax - dipole_efieldx)) * 0.393456
        ) / (volume * np.power(1.8897259886, 3) * 0.0005) + eps_inf_xx
        eps_0_yy = (
            4 * np.pi * (np.absolute(dipole_efieldy_relax - dipole_efieldy)) * 0.393456
        ) / (volume * np.power(1.8897259886, 3) * 0.0005) + eps_inf_yy
        eps_0_zz = (
            4 * np.pi * (np.absolute(dipole_efieldz_relax - dipole_efieldz)) * 0.393456
        ) / (volume * np.power(1.8897259886, 3) * 0.0005) + eps_inf_zz
